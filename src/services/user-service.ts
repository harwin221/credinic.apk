 ...
 */
export async function updateUserPassword(userId: string, newPassword: string, oldPassword?: string, isAdminChange: boolean = false): Promise<void> {
  const result = await updateUserPasswordServer(userId, newPassword, oldPassword, isAdminChange);
  if (!result.success) {
